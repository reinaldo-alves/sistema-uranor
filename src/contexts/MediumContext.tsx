    ...
    const loadMedium = (token: string) => {
        api.get('/medium/get-mediuns', {headers:{Authorization: token}}).then(({ data }) => {
            const medium = data.medium.map((item: IMediumAPI) => ({
                ...
                adjOrigem: item.adjOrigem === null ? '' : item.adjOrigem,
                ...
                ministro: item.ministro === null ? '' : item.ministro,
                cavaleiro: item.cavaleiro === null ? '' : item.cavaleiro,
                ...
                ninfa: item.ninfa === null ? '' : item.ninfa,
                mestre: item.mestre === null ? '' : item.mestre,
                padrinho: item.padrinho === null ? '' : item.padrinho,
                madrinha: item.madrinha === null ? '' : item.madrinha,
                afilhado: item.afilhado === null ? '' : item.afilhado,
                comando: item.comando === 1 ? true : false,
                janata: item.janata === 1 ? true : false,
                lVermelha: item.lVermelha === 1 ? true : false,
                presidente: item.presidente === 1 ? true : false,
                vicePres: item.vicePres === 1 ? true : false,
