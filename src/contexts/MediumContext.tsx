    ...
    const loadMedium = (token: string) => {
        api.get('/medium/get-mediuns', {headers:{Authorization: token}}).then(({ data }) => {
            const medium = data.medium.map((item: IMediumAPI) => ({
                ...
                dtTestD: item.dtTestD === null ? '' : item.dtTestD,
                dtTestA: item.dtTestA === null ? '' : item.dtTestA,
                dtEmplD: item.dtEmplD === null ? '' : item.dtEmplD,
                dtEmplA: item.dtEmplA === null ? '' : item.dtEmplA,
                dtInicD: item.dtInicD === null ? '' : item.dtInicD,
                dtInicA: item.dtInicA === null ? '' : item.dtInicA,
                dtElevD: item.dtElevD === null ? '' : item.dtElevD,
                dtElevA: item.dtElevA === null ? '' : item.dtElevA,
