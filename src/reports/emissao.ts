...
export const emissaoText = (medium: IMedium, mediuns: Array<IMedium>, ministros: Array<IMentor>, cavaleiros: Array<ICavaleiro>, guias: Array<IMentor>, adjuntos: Array<IAdjunto>, templos: Array<ITemplo>, falMiss: Array<IFalange>) => {
    ...
    const falMest = medium.falMest === 'Estrela Candente' && medium.classif !== 'Adjunto Koatay 108 Herdeiro Triada Harpásios 7° Raio Adjuração Arcanos Rama 2000' && mestre?.classif !== 'Adjunto Koatay 108 Herdeiro Triada Harpásios 7° Raio Adjuração Arcanos Rama 2000' ? 'ESTRELA CANDENTE, DO MINISTRO JANARÃ' : medium.falMest;
    ...
    const termoFalangeJaguar = medium.classif === 'Adjunto Koatay 108 Herdeiro Triada Harpásios 7° Raio Adjuração Arcanos Rama 2000' && medium.devas && medium.falMiss === 6 ? '' : medium.falMiss ? `${medium.regente? 'REGENTE ' : ''}${medium.falMiss === 6 ? `${medium.devas? 'MISSIONÁRIO' : 'MAGO MISSIONÁRIO' }` : medium.falMiss === 7 ? 'PRÍNCIPE MAYA DESTE AMANHECER' : ''}${medium.classif !== 'Adjunto Koatay 108 Herdeiro Triada Harpásios 7° Raio Adjuração Arcanos Rama 2000' ? ` DO ADJUNTO ${medium.adjDevas} KOATAY 108 MESTRE ${adjDevasNome}` : ''}. ${medium.falMiss === 7 && medium.classif !== 'Adjunto Koatay 108 Herdeiro Triada Harpásios 7° Raio Adjuração Arcanos Rama 2000' ? 'NA REGÊNCIA DO MINISTRO YURICY. ' : ''}`: '';

    //Retorna o termo que cita a classificação do adjunto de origem do mestre, se não for esperança
    const termoClassifOrigem = medium.classif === 'Adjunto Koatay 108 Herdeiro Triada Harpásios 7° Raio Adjuração Arcanos Rama 2000' || medium.presidente === 'Presidente' ? '. ' : adjOrigemClassif === 'Arcanos' ? `, NA LINHA DO MESTRE ${adjOrigemNome}, 7º RAIO ADJURAÇÃO ARCANOS RAMA 2000. ` : `, NA LINHA DO MESTRE ${adjOrigemNome}, RAIO ADJURAÇÃO RAMA 2000. `;

    //Retorna o termo que cita a classificação do adjunto de origem do mestre, se for esperança
    const termoClassifOrigemEsp = medium.classif === 'Adjunto Koatay 108 Herdeiro Triada Harpásios 7° Raio Adjuração Arcanos Rama 2000' || medium.presidente === 'Presidente' ? '. ' : `, NO ADJUNTO ${ministros.find((min: IMentor) => min.id === adjuntos.find((item: IAdjunto) => item.adjunto_id === presidente?.adjunto_id)?.ministro)?.nome} KOATAY 108 NA LINHA DO MESTRE ${presidente?.nome}, ${presidente?.classif === 'Arcanos' ? `7º RAIO ADJURAÇÃO ARCANOS RAMA 2000. ` : `RAIO ADJURAÇÃO RAMA 2000. `}`;
    ...
    const termoEmMissao = presidente?.adjunto_id !== medium.adjOrigem && !esperanca && medium?.presidente !== 'Presidente' && medium?.classif !== 'Adjunto Koatay 108 Herdeiro Triada Harpásios 7° Raio Adjuração Arcanos Rama 2000' && afilhado?.classif !== 'Adjunto Koatay 108 Herdeiro Triada Harpásios 7° Raio Adjuração Arcanos Rama 2000' && afilhado?.presidente !== 'Presidente'? `EM MISSÃO DO ADJUNTO ${ministros.find((min: IMentor) => min.id === adjuntos.find((item: IAdjunto) => item.adjunto_id === presidente?.adjunto_id)?.ministro)?.nome} KOATAY 108 MESTRE ${presidente?.nome}. ` : '';
    ...
    const termoJanata = medium.comando.includes('Janatã') && medium.classif !== 'Adjunto Koatay 108 Herdeiro Triada Harpásios 7° Raio Adjuração Arcanos Rama 2000' ? 'COMANDANTE DO ADJUNTO JANATÃ MESTRE JOSÉ LUIZ. ' : '';

    //Retorna o termo 'Vindo do 7° Raio' ou '7° Raio', dependendo do caso
    const termoSetimo = medium.classif === '7° Raio Autorizado Taumantes Raio Rama Adjuração' ? '7° Raio, ' : medium.classif === 'Adjunto Koatay 108 Herdeiro Triada Harpásios 7° Raio Adjuração Arcanos Rama 2000' || medium.presidente === 'Presidente' ? '' : 'VINDO DO 7° RAIO, ';

    //Retorna a classificação do mestre
    const termoClassifSol = `${medium.trinoSar && medium.trinoSol? `TRINO SARDYOS SOLITÁRIO ${medium.trinoSol} ` : medium.trinoSol? `TRINO SOLITÁRIO ${medium.trinoSol} ` : medium.trinoSar? 'TRINO SARDYOS ' : ''}${medium.ministro? medium.classif === 'Adjunto Regente Koatay 108 Taumantes Raio Rama Adjuração' ? `ADJUNTO REGENTE ${ministros.find((item: IMentor) => item.id === medium.ministro)?.nome} KOATAY 108 TAUMANTES RAIO RAMA ADJURAÇÃO` : `ADJUNTO ${ministros.find((item: IMentor) => item.id === medium.ministro)?.nome}${medium.classif.replace(/Adjunto/g, '')}` : medium.classif} MESTRE ${medium.nomeEmissao}. `;
    ...
    const estrelaCav = medium.trinoSar ? 'RANDAY' : medium.classif === 'Adjunto Koatay 108 Herdeiro Triada Harpásios 7° Raio Adjuração Arcanos Rama 2000' || medium.presidente === 'Presidente' ? 'RANDYÊ' : 'ANDAY';

    //Retorna o termo que cita o cavaleiro na emissão do mestre
    const termoCavaleiroSol = medium.cavaleiro ? `1° CAVALEIRO DA LANÇA ${cavaleiros.find((item: IMentor) => item.id === medium.cavaleiro)?.nome} VERDE ${estrelaCav}` : `MEU 1° CAVALEIRO DA LANÇA VERDE ${estrelaCav}`;

    //Retorna o termo de emissão (barras)
    const termoBarras = medium.classif === 'Adjunto Koatay 108 Herdeiro Triada Harpásios 7° Raio Adjuração Arcanos Rama 2000' ? '– 0 – X – X –' : medium.presidente === 'Presidente' ? '– 0 – X –' : '– 0 –';
    ...
    const termoPadrinhoArcanos = afilhado && medium.herdeiro !== medium.afilhado? `PADRINHO DO ADJUNTO ${ministros.find((item: IMentor) => item.id === afilhado.ministro)?.nome} KOATAY 108 MESTRE ${afilhado.nomeEmissao}, 7° RAIO ADJURAÇÃO ARCANOS RAMA 2000. ` : '';

    //Retorna a classificação do mestre
    const termoClassifLua = `${medium.trinoSar? 'TRINO SARDYOS ' : ''}${medium.ministro? medium.classif === '5° Yurê Adjunto Regente Cautanenses Raio Rama Ajanã' ? `5° YURÊ ADJUNTO REGENTE ${ministros.find((item: IMentor) => item.id === medium.ministro)?.nome} CAUTANENSES RAIO RAMA AJANÃ` : `5° YURÊ ADJUNTO ${ministros.find((item: IMentor) => item.id === medium.ministro)?.nome}${medium.classif.replace(/5° Yurê Adjunto/g, '')}` : medium.classif} MESTRE ${medium.nomeEmissao}. `;
