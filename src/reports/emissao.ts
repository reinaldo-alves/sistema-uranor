...
export const emissaoText = (medium: IMedium, mediuns: Array<IMedium>, ministros: Array<IMentor>, cavaleiros: Array<ICavaleiro>, guias: Array<IMentor>, adjuntos: Array<IAdjunto>, templos: Array<ITemplo>, falMiss: Array<IFalange>) => {
    ...
    const termoOrigemSol = ministros.find((min: IMentor) => min.id === adjuntos.find((item: IAdjunto) => item.adjunto_id === medium.adjOrigem)?.ministro)?.id === afilhado?.ministro && adjOrigemNome === afilhado?.nomeEmissao ? ' ' : afilhado?.presidente === 'Presidente' || afilhado?.classif === 'Adjunto Koatay 108 Herdeiro Triada Harpásios 7º Raio Adjuração Arcanos Rama 2000'? `NA ORDEM DO MINISTRO ${adjOrigemMin}. ` : esperanca? `NA ORDEM DO MINISTRO ${adjOrigemMin}, ${medium.falMiss ? 'NO' : 'DO'} ADJUNTO ${ministros.find((min: IMentor) => min.id === adjuntos.find((item: IAdjunto) => item.adjunto_id === presidente?.adjunto_id)?.ministro)?.nome} KOATAY 108 MESTRE ${presidente?.nome}. ` : `${medium.falMiss ? 'NO' : 'DO'} ADJUNTO ${adjOrigemMin} KOATAY 108 MESTRE ${adjOrigemNome}. `
    ...
    //Adiciona o termo NA ORDEM DO MINISTRO... na emissão do mestre, exceto de padrinhos de seus próprios Adjuntos de Origem.
    const termoOrigemAjana = ministros.find((min: IMentor) => min.id === adjuntos.find((item: IAdjunto) => item.adjunto_id === medium.adjOrigem)?.ministro)?.id !== afilhado?.ministro || adjOrigemNome !== afilhado?.nomeEmissao ? `NA ORDEM DO MINISTRO ${adjOrigemMin}` : '';
